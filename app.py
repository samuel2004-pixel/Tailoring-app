...
def _attendance_register_data(year, mon, student_id):
    ...
    the centre's paper 'Staff Attendance' register: Sl.No / Name /
...
def export_attendance_csv():
    ...
    si = io.StringIO()
    writer = csv.writer(si)
    writer.writerow([f"Staff Attendance for the Month of {month_label}"])
    writer.writerow([])
...
def export_attendance_pdf():
    ...
    day of the month, laid out like the centre's paper 'Staff Attendance'
    ...
    story.append(Paragraph(f"Staff Attendance for the Month of {month_label}", styles["Heading2"]))
